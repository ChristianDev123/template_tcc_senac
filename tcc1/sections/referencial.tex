...
Segundo \citeonline{Camargo2022}, em torno de 60\% do recurso energético de sistemas \gls{iot} são consumidos na comunicação, a escolha efetiva de protocolo de comunicação e otimização de parâmetros como os citados na tabela~\ref{tab:parametros_lora}, torna-se uma tarefa de suma importância no desenvolvimento de um sistema \gls{iot} otimizado de baixo custo.
...
O pipeline de trabalho consistirá na criação e tratamento do dataset utilizando como base o conjunto público Water Meters Dataset por \citeonline{kucev2020}, associado a capturas locais do próprio sensor, treinamento supervisionado e aplicação do modelo no sistema \gls{iot}.
