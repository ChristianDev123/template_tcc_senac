...
Uma das principais incógnitas levantadas por este artigo é a viabilidade do envio das imagens geradas em nós na borda da rede ao getaway (nó central). Como apresentado por \cite{almeida2022iot}, a viabilidade na propagação de dados não estruturados via rede LoRaWan, é algo sim viável, mas com a ressalva de limitações de bits enviados por pacote, maior latência na disponibilização dos dados e a aplicação de algoritmos que orquestra todo o recebimento e envio de dados pela rede.
...
A solução proposta fundamenta-se no desenvolvimento e na análise de um \textbf{Sistema Telemétrico de Baixo Custo para Digitalização de Hidrómetros Analógicos}, integrando dispositivos de borda, comunicação de longo alcance e processamento em nuvem. O diferencial desta pesquisa reside na investigação experimental de duas arquiteturas distintas de visão computacional, visando identificar a configuração ideal para cenários de restrição energética e de largura de banda.
