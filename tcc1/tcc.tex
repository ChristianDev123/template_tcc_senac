...
Atualmente, o método mais estudado e aplicado como solução na transmissão de dados no campo da Internet das Coisas (IOT) é a abordagem de computação em borda (Edge Computing), onde torna-se necessário o desenvolvimento de modelos específicos de visão computacional baseados em aprendizado de máquina (Machine learning). Como contraposto, há um método menos difundido na literatura que é a abordagem de processamento centralizado (Cloud Computing), onde os dados não estruturados passam por tratamentos e são enviados via rede para um nó que possui recurso abundantes de processamento e memória com o objetivo de realizar o reconhecimento de caracteres utilizando Frameworks já estabelecidos no mercado e na comunidade.

Diante de tais métodos, revelam-se intrigantes questionamentos acerca da viabilidade de implementação do método centralizado na problemática hídrica, e em especial, sob redes de baixa largura de banda, e levantar quais suas vantagens e desvantagens em relação ao método convencionalmente implementado.
