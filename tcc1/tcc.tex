...
A solução proposta fundamenta-se no desenvolvimento e na análise de um \textbf{Sistema Telemétrico de Baixo Custo para Digitalização de Hidrómetros Analógicos}, integrando dispositivos de borda, comunicação de longo alcance e processamento em nuvem. O diferencial desta pesquisa reside na investigação experimental de duas arquitecturas distintas de visão computacional, visando identificar a configuração ideal para cenários de restrição energética e de largura de banda.
...
\begin{enumerate}
    \item \textbf{Processamento Local (Edge OCR):} O dispositivo de borda realiza a inferência da imagem localmente (utilizando modelos optimizados). Neste modo, apenas o valor numérico da leitura é transmitido.
