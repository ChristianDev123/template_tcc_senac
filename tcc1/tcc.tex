...
Para a implementação do OCR na borda, será adotada a abordagem TinyML utilizando a biblioteca TensorFlow Lite for Microcontrollers. Essa ferramenta apresenta baixo custo computacional e de alocação de memória, mantendo um bom desempenho de inferência, além de possuir compatibilidade nativa com o ESP-IDF, conforme demonstrado por \cite{david2021}.
...
Uma das principais incógnitas levantadas por este artigo é a viabilidade do envio das imagens geradas em nós na borda da rede ao gateway (nó central). Como apresentado por \citeonline{almeida2022iot}, a propagação de dados não estruturados via rede LoRaWAN é algo viável, mas com a ressalva de limitações de bits por pacote, maior latência na disponibilização dos dados e a necessidade de algoritmos que orquestrem todo o recebimento e envio de dados pela rede, incluindo a fragmentação da imagem em múltiplos pacotes e sua posterior remontagem no destino.
 
Diante disso, casa-se perfeitamente a implementação de algoritmos para a redução do tamanho de imagens, a considerar que este é um dos fatores mais influentes na execução de tal abordagem. Um ponto apresentado por \citeonline{almeida2022iot} é a aplicação de algoritmos de compressão como o JPEG e a transformação de imagens em escala de cinza, reduzindo assim as informações a serem enviadas via rede. Outros algoritmos avaliados como benéficos para a otimização desta abordagem são o Bounding Box, ou caixa delimitadora, isto é, o recorte da imagem em uma área de interesse, e a Dilação Morfológica, algoritmo utilizado para unificar conteúdo dentro de uma imagem criando uma ``ilha de informação''. Ambos são amplamente utilizados em modelos de visão computacional com o objetivo de encontrar núcleos de informação em imagens.
 
Uma das principais preocupações técnicas no envio de imagens via LoRaWAN diz respeito aos limites regulatórios de \textit{duty cycle} e ao tamanho máximo de \textit{payload} por pacote. Redes LoRaWAN operando na faixa sub-GHz (como a EU868, padrão em grande parte do mundo) estão sujeitas a um \textit{duty cycle} de 1\%, o que equivale a um orçamento de transmissão de apenas 36 segundos por hora, ou 864 segundos por dia, de acordo com \citeonline{jebril2018overcoming}. Além disso, o tamanho máximo de \textit{payload} por pacote varia conforme o \textit{Spreading Factor} (SF) utilizado. Com SF9 e largura de banda de 125~kHz, o limite é de 115 bytes brutos por pacote. Descontando o \textit{overhead} do protocolo LoRaWAN, estimado em 30 bytes (considerando cabeçalho MAC, porta de aplicação e código de integridade), restam 85 bytes úteis por pacote para os dados da imagem.

A escolha do \textit{Spreading Factor} (SF) é determinante nesse contexto, pois influencia diretamente tanto o tempo no ar de cada pacote quanto o tempo de calendário necessário para concluir a transmissão completa da imagem, respeitando o intervalo mínimo imposto pelo \textit{duty cycle} entre pacotes consecutivos. SFs elevados como SF11 e SF12, embora aumentem o alcance, resultam em tempos de calendário de 2,6 e 4,7 horas, respectivamente, para a transmissão de uma única imagem, o que os torna inviáveis para um sistema de monitoramento com capturas periódicas. SF10 apresenta tempo no ar de 43,7~s por transmissão, valor que supera o orçamento horário de 36~s caso os pacotes sejam enviados em sequência imediata, exigindo espaçamento de mais de uma hora entre o início e o fim da transmissão. \citeonline{jebril2018overcoming} demonstraram experimentalmente que SF9 é suficiente para cobrir distâncias de até 4~km sem perda de pacotes, representando o ponto de equilíbrio entre alcance, tempo no ar (23,4~s por imagem) e tempo de calendário (~39 minutos), com o tempo no ar mantido dentro do orçamento horário de 36~s. Por essas razões, SF9 foi adotado como parâmetro de referência neste trabalho.

É importante distinguir o tempo de calendário da transmissão do tempo efetivo em que o sistema permanece ativo. O intervalo de aproximadamente 39 minutos representa o tempo total decorrido para a transmissão de uma imagem, mas não implica que o dispositivo permaneça ligado durante todo esse período. Em uma implementação orientada à eficiência energética, o microcontrolador pode entrar em modo de \textit{deep sleep} entre o envio de cada pacote, acordando apenas para transmitir o pacote seguinte. Para SF9, o intervalo mínimo entre pacotes imposto pelo \textit{duty cycle} de 1\% é de aproximadamente 60,9 segundos, durante os quais o rádio e o processador podem permanecer inativos. Dessa forma, o tempo efetivo em que o sistema permanece acordado por transmissão de imagem se reduz a aproximadamente 25 segundos, considerando o tempo no ar de cada pacote (615,4~ms) acrescido do tempo de processamento e gerenciamento da sequência de envio. Para o cenário de duas capturas diárias, o sistema opera ativamente por cerca de 50 segundos de transmissão de rádio e aproximadamente 4 segundos de ativação da câmera, permanecendo em \textit{deep sleep} durante o restante do dia. Essa característica é compatível com operação por bateria a longo prazo, desde que o consumo em modo de \textit{deep sleep} do microcontrolador utilizado seja suficientemente baixo, requisito atendido por plataformas como o ESP32, cujo consumo nesse modo pode ser inferior a 10~$\mu$A..
 
Para realizar a estimativa, utilizou-se uma imagem obtida diretamente da câmera ESP32-CAM na resolução mínima disponível no dispositivo ($160 \times 120$ pixels), em escala de cinza, com o parâmetro de qualidade configurado em 4 na escala do ESP32-CAM (escala invertida de 4 a 63, onde valores menores correspondem a menor compressão e maior fidelidade de imagem). A imagem resultante, já no formato JPEG gerado pelo próprio sensor, apresentou tamanho de aproximadamente 3,10~KB (3.182 bytes). Com 3.182 bytes de dados e 85 bytes úteis por pacote, são necessários 38 pacotes para transmitir a imagem completa, conforme a Equação~\ref{eq:pacotes}:
...
O tempo no ar por pacote foi calculado utilizando a fórmula oficial do LoRa para SF9, largura de banda de 125~kHz, \textit{coding rate} 4/5 e preâmbulo padrão de 8 símbolos. O \textit{coding rate} 4/5 foi escolhido por representar o menor \textit{overhead} de redundância disponível no padrão LoRa, resultando no menor tempo no ar possível para o cenário proposto, conforme as Equações~\ref{eq:tsym} a \ref{eq:tpacote}:
 
\begin{equation}
  \label{eq:tsym}
  T_{\text{símbolo}} = \frac{2^{SF}}{BW} = \frac{2^9}{125{.}000} = 4{,}096 \text{ ms}
\end{equation}
...
\noindent onde $PL = 115$ bytes (payload total por pacote), $SF = 9$ e $CR = 1$
(para \textit{coding rate} 4/5), resultando em:
 
\begin{equation}
  n_{\text{sym}}^{\text{payload}} = 8 + \left\lceil \frac{8 \times 115 - 4 \times 9 + 28 + 16}{4 \times 9} \right\rceil \times 5
  = 8 + \left\lceil \frac{928}{36} \right\rceil \times 5
  = 8 + 26 \times 5 = 138 \text{ símbolos}
\end{equation}
 
\begin{equation}
  \label{eq:tpacote}
  T_{\text{pacote}} = \bigl(n_{\text{preamble}} + 4{,}25 + n_{\text{sym}}^{\text{payload}}\bigr) \times T_{\text{símbolo}}
  = \left(8 + 4{,}25 + 138\right) \times 4{,}096 \approx 615{,}4 \text{ ms}
\end{equation}
 
O tempo total de transmissão estimado para os 38 pacotes é, portanto:
 
\begin{equation}
  \label{eq:ttotal}
  T_{\text{total}} = 38 \times 615{,}4 \approx 23{,}4 \text{ s}
\end{equation}
 
Com base nesses parâmetros, é possível calcular o impacto de cada transmissão sobre
o orçamento diário de \textit{duty cycle}:
 
\begin{itemize}
  \item 1 foto por dia: 23,4~s utilizados de 864~s disponíveis $\rightarrow$ 2,7\% do orçamento diário.
  \item 2 fotos por dia: 46,8~s utilizados de 864~s disponíveis $\rightarrow$ 5,4\% do orçamento diário.
\end{itemize}
 
Esses valores demonstram que, para o caso de uso proposto, monitoramento com uma a duas capturas diárias  o envio de imagens via LoRaWAN é viável dentro dos limites regulatórios, com margem superior a 94\% do orçamento disponível. O intervalo mínimo entre transmissões, caso o sistema operasse em modo contínuo, seria de aproximadamente 39 minutos, calculado conforme a Equação~\ref{eq:intervalo}:
 
\begin{equation}
  \label{eq:intervalo}
  T_{\text{intervalo}} = \frac{T_{\text{total}}}{DC} = \frac{23{,}4}{0{,}01} = 2{.}340 \text{ s} \approx 39 \text{ min}
\end{equation}
 
\citeonline{jebril2018overcoming} confirmam experimentalmente a viabilidade do envio de imagens sobre a camada física LoRa, tendo transmitido imagens com sucesso a distâncias de até 6~km com SF12. Em seus experimentos com SF9, os autores registraram um tempo médio por pacote de aproximadamente 613~ms --- valor empiricamente próximo aos 615,4~ms obtidos pela fórmula analítica aplicada neste trabalho, o que corrobora a consistência do modelo de estimativa adotado. É relevante destacar que \citeonline{jebril2018overcoming} optaram por operar diretamente sobre a camada física LoRa (PHY), contornando o protocolo LoRaWAN, em razão das limitações impostas pela camada MAC ao lidar com fragmentação e remontagem de dados de imagem, abordagem distinta da adotada neste trabalho, que opera sobre LoRaWAN e, portanto, está sujeita às restrições regulatórias de \textit{duty cycle} aqui modeladas.
 
A Tabela~\ref{tab:parametros} resume os parâmetros utilizados na estimativa e os resultados obtidos.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{ll}
    ...
    \textit{Spreading Factor}               & SF9                         \\
    Largura de banda                        & 125~kHz                     \\
    \textit{Coding rate}                    & 4/5                         \\
    \textit{Overhead} LoRaWAN               & 30 bytes                    \\
    \textit{Payload} útil por pacote        & 85 bytes                    \\
    Pacotes necessários                     & 38                          \\
    Tempo no ar por pacote                  & $\approx$615,4~ms           \\
    Tempo no ar total                       & $\approx$23,4~s             \\
    Orçamento diário (\textit{duty cycle})  & 864~s/dia                   \\
    Uso para 1 foto/dia                     & 2,7\%                       \\
    Uso para 2 fotos/dia                    & 5,4\%                       \\
    Intervalo mínimo (modo contínuo)        & $\approx$39 min             \\
    \bottomrule
  \end{tabular}
  \fonte{Os autores (2025).}
\end{table}
 
Conclui-se que o envio de imagens via LoRaWAN, nas condições descritas, é tecnicamente viável e compatível com os limites regulatórios para o caso de uso de monitoramento com baixa frequência de captura. A margem disponível no orçamento de \textit{duty cycle} é superior a 94\% mesmo no cenário de duas capturas diárias, e o modelo analítico adotado é corroborado empiricamente pelos resultados de \citeonline{jebril2018overcoming}. O principal requisito de implementação decorrente desta análise é a adoção de um algoritmo de fragmentação e remontagem de pacotes na camada de aplicação, necessário para viabilizar a transmissão sequencial dos 38 pacotes que compõem a imagem.
...
Como forma de contorno, aplicando os algoritmos de escala de cinza (reduzindo a imagem à 1 canal de cor) e posteriormente aplicando o algoritmo de limiarização adaptativa de Otsu (apresentado resultado na imagem abaixo), onde o foco é enviar apenas a informação da imagem e não os detalhes, apresenta-se uma redução substancial no tamanho das imagens.

\begin{figure}[htbp]
    \centering % Centraliza a imagem na página
    \includegraphics[width=0.7\textwidth]{Binaria.png}
    \caption{Resultado da limiarização adaptativa de Otsu aplicada ao hidrómetro.}
    \label{fig:resultado_binario}
\end{figure}

Conforme apresentado na tabela 2, aplicando os algoritmos citados anteriormente, alcançamos significativos avanços em relação a redução do tamanho da imagem enviada.

\begin{table}[h!]
    \centering
    \caption{Níveis de compressão utilizando algoritmo PNG: 320 x 240 px (Média Consolidada em bytes)}
    \vspace{2mm}
    \begin{tabular}{cccc}
        \toprule
        \textbf{Nível} & \textbf{Original (3 ch)} & \textbf{Escala Cinza (1 ch)} & \textbf{Binarizada (1 ch)} \\ 
        \midrule
        0 & 231.074 & 77.226 & 77.226 \\
        1 & 62.523  & 27.251 & 5.298  \\
        2 & 61.164  & 26.381 & 4.922  \\
        3 & 57.720  & 25.474 & 4.531  \\
        4 & 56.776  & 24.530 & 4.044  \\
        5 & 54.664  & 23.793 & 3.822  \\
        6 & 52.669  & 23.254 & 3.528  \\
        7 & 51.946  & 23.044 & 3.425  \\
        8 & 50.805  & 22.737 & 3.158  \\
        9 & 50.325  & 22.639 & 2.971  \\
        \bottomrule
    \end{tabular}
\end{table}

Ao relevar dos dados apresentados, esperamos que aplicando os tais algoritmos conseguiremos reduzir as imagens geradas pelo ESP32-CAM em até 95\% em relação ao seu tamanho original.

A escolha da compressão por PNG foi realizada por conta da melhor adaptabilidade do algoritmo para comprimir imagens binarizadas, coisa que o JPG não lida tão bem, não apresentando ganhos significativos na binarização da imagem.

Conforme apresentado por \citeonline{jebril2018overcoming}, uma imagem com tamanho aproximado de 3000 bytes levaria algo entorno de 22 segundos para ser completamente enviada ao destinatário, levando em consideração todos os tratamentos aplicados a imagem que estamos adotando, espera-se conseguirmos realizar o envio de imagens sob as mesmas configurações de SF em um tempo menor ou igual ao demonstrado por Jebril e demais.

\subsubsection{Resultados de Compressão JPG em OpenCV}

% --- Tabela 1 ---
\begin{table}[ht]
    \centering
    \caption{Tamanho do arquivo para a imagem 20260511203832.jpg (160x120 px)}
    \begin{tabular}{@{}cccc@{}}
        \toprule
...
\begin{table}[ht]
    \centering
    \caption{Tamanho do arquivo para a imagem 20260511204204.jpg (160x120 px)}
    \begin{tabular}{@{}cccc@{}}
        \toprule
        Qualidade & Original (Bytes) & Escala Cinza (Bytes) & Binarizada (Bytes) \\ \midrule
        0  & 1123 & 722  & 1467 \\
...
\begin{table}[ht]
    \centering
    \caption{Tamanho do arquivo para a imagem 20260511204422.jpg (160x160 px)}
    \begin{tabular}{@{}cccc@{}}
        \toprule
        Qualidade & Original (Bytes) & Escala Cinza (Bytes) & Binarizada (Bytes) \\ \midrule
        0  & 1106 & 711  & 747  \\
