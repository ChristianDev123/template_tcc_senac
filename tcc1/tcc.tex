...
Para realizar a estimativa, considerou-se uma imagem capturada pela câmera ESP32-CAM na resolução mínima disponível no dispositivo ($160 \times 120$ pixels), em escala de cinza, com compressão JPEG de qualidade 25. Essa configuração produziu, em validação empírica, uma imagem comprimida de aproximadamente 3,10~KB (3.182 bytes), representando uma taxa de compressão de aproximadamente 83\% em relação ao tamanho bruto de 18,8~KB --- valores compatíveis com os observados por \citeonline{almeida2022iot}. Com 3.182 bytes de dados e 85 bytes úteis por pacote, são necessários 38 pacotes para transmitir a imagem completa, conforme a Equação~\ref{eq:pacotes}:
...
O tempo no ar por pacote foi calculado utilizando a fórmula oficial do LoRa para SF9, largura de banda de 125~kHz, \textit{coding rate} 4/5 e preâmbulo padrão de 8 símbolos, conforme as Equações~\ref{eq:tsym} a \ref{eq:tpacote}:
...
\noindent onde $PL = 115$ bytes (payload total), $SF = 9$, $CR = 1$ (para \textit{coding rate} 4/5),
resultando em:

\begin{equation}
  n_{\text{sym}}^{\text{payload}} = 8 + \left\lceil \frac{928}{36} \right\rceil \times 5 = 8 + 26 \times 5 = 138 \text{ símbolos}
\end{equation}

\begin{equation}
  \label{eq:tpacote}
  T_{\text{pacote}} = \left(n_{\text{preamble}} + 4{,}25 + n_{\text{sym}}^{\text{payload}}\right) \times T_{\text{símbolo}}
...
Com base nesses parâmetros, é possível calcular o impacto de cada transmissão sobre o orçamento diário de \textit{duty cycle}:
...
Conclui-se que o envio de imagens comprimidas via LoRaWAN, nas condições descritas, é tecnicamente viável e compatível com os limites regulatórios para o caso de uso de monitoramento com baixa frequência de captura.
