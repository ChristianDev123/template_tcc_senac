...
Embora a transmissão de imagens via rede \gls{lorawan} apresente limitações críticas de largura de banda e maior latência na disponibilização dos dados, esta permite o uso de \textit{frameworks} de OCR mais robustos e estabelecidos, como o motor Tesseract. 
...
Comparar a viabilidade técnica, a acurácia e o consumo energético entre o processamento de OCR realizado localmente no \gls{esp32cam} (abordagem Edge Computing) e a inferência OCR realizada remotamente em servidor na nuvem (Cloud), utilizando a rede \gls{lorawan} como meio de transmissão.

\subsection{Objetivos secundários}

\begin{enumerate}
    \item Implementar os dois fluxos de processamento que compõem o experimento comparativo: o fluxo Edge, em que o OCR ocorre localmente no \gls{esp32cam}; e o fluxo Cloud, em que a imagem é transmitida para processamento remoto. % 1. construção do experimento
...
Ao relevar dos dados apresentados, esperamos conseguir reduzir as imagens geradas pelo \gls{esp32cam} em até 60\% em relação ao seu tamanho original, utilizando apenas os métodos apresentados.
...
\subsection{Envio de Imagens via \gls{lorawan}}
...
\begin{itemize}
    ...
    \item \textbf{Protocolo de Comunicação:} Utilização de rede \gls{lorawan} para transmissão de longa distância e baixo consumo.
\end{itemize}
...
Esta abordagem dual permitirá a recolha de métricas quantitativas sobre a vida útil estimada da bateria, a acurácia do reconhecimento em diferentes condições de luminosidade e a fiabilidade da transmissão de pacotes pesados em redes \gls{lorawan}. Assim, a solução deixa de ser meramente uma implementação técnica para tornar-se uma análise de viabilidade para sistemas de \textit{Smart Metering} em larga escala.
...
\subsection{Viabilidade do Envio de Imagens via \gls{lorawan}}
...
\gls{lorawan}. O cenário de referência primário é o padrão brasileiro 
...
Uma das principais preocupações técnicas no envio de imagens via \gls{lorawan} diz
respeito aos limites regulatórios de transmissão e ao tamanho máximo de \textit{payload}
por pacote. No Brasil, o padrão regulamentado pela ANATEL para redes \gls{lorawan} é o
...
\gls{esp32cam} na resolução mínima disponível no dispositivo ($160 \times 120$ pixels),
em escala de cinza, com o parâmetro de qualidade configurado em 4 na escala do
\gls{esp32cam} (escala invertida de 4 a 63, onde valores menores correspondem a menor
...
(3.182~bytes). Descontando o \textit{overhead} do protocolo \gls{lorawan}, estimado em
...
funcionamento do sistema. Na prática, redes \gls{lorawan} em cenários urbanos operam com
...
do sistema em qualquer implantação \gls{lorawan} em território brasileiro.
...
a camada física LoRa (PHY), contornando o protocolo \gls{lorawan}, em razão das
limitações impostas pela camada MAC ao lidar com fragmentação e remontagem de dados
de imagem, abordagem distinta da adotada neste trabalho, que opera sobre \gls{lorawan}
...
Conclui-se que o envio de imagens via \gls{lorawan}, nas condições descritas, é 
